...
#define TERRAIN_FAR 1000000.0
#define TERRAIN_HEIGHT 4000.0 // terrain amplitude
#define TERRAIN_LIFT  -2400.0 // how much to lift/drop the entire terrain
#define TERRAIN_SAMPLE_SCALE 0.00002
...
#define WATER_COLOR vec3(0.96, 0.9, 0.98)
...
#define WATER_CHOPPY      0.7 // smaller beachfront-type waves, they travel in parallel
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	if (d < t)
	{
		...
		intersec.emission = vec3(1.0,1.0,1.0);
...
//-----------------------------------------------------------------------
{
        ...
        for (int bounces = 0; bounces < 3; bounces++)
	{
		...
		if (intersec.type == TERRAIN)
		{
			...
			vec3 rockColor0 = vec3(0.2, 0.2, 0.9) * 0.31 * rockNoise;
			vec3 rockColor1 = vec3(0.9, 0.2, 0.2) * rockNoise;
	...
	float fogStartDistance = TERRAIN_FAR * 0.1;
	...
	return max(vec3(0.8, 0.0, 0.3), accumCol); // prevents black spot artifacts appearing in the water
...
void main( void )
{
	...
	vec3 pixelColor = hueShift(CalculateRadiance(ray, uSunDirection),uColorChange);
