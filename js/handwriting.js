...
const STROKE_WIDTH = 0.76;

let r; let e; let t; const a = (r) => (e) => {
    const t = Y(K(e));
    for (let a = 0; a < K(e); a++) t[a] = r(e[a]);
    return t;
}; const l = a((r) => Q(r)); const o = a((r) => 1 / (1 + N(-r))); const n = a((r) => Q(1 + N(r))); const v = a((r) => {
    const e = N(2 * r);
    return (e - 1) / (e + 1);
}); const i = (r) => (e, t) => {
    ...
    return l;
}; const u = i((r, e) => r + e); const f = i((r, e) => r - e); const s = i((r, e) => r * e); const h = i((r, e) => r / e); const d = (r) => {
    const e = Y(K(r)); let
        t = 0;
    ...
    const [t, a, l, o] = e; const n = t[0]; const
        v = Y(n);
    for (let e = 0; e < n; e++) {
        const i = o[e]; const u = o[e + 1]; let
            f = 0;
    ...
    if (t == 1) {
        ...
        var n = $.y;
        var i = $.p;
    } else if (t == 2) n = $.w, i = $.q, a = e.b, l = e.e; else n = $.r, i = $.f, a = e.c, l = e.f;
    r = w(r, l);
    const f = u(g(r, n), i); const [h, d, c, p] = b(f, 4); const m = u(s(o(c), a), s(o(h), v(d))); const
        M = s(o(p), v(m));
    return t == 1 ? (e.a = m, e.d = M) : t == 2 ? (e.b = m, e.e = M) : (e.c = m, e.f = M), M;
}; const A = (r) => {
    r = [0, ...r, 0], r = Y(r);
    let e; let t = ((r, e) => {
        const t = [K(r) / G - 2, G]; const
            a = Y(t[0] * t[1]);
        for (let n = 0; n < t[0]; n++) {
        ...
        return a;
    })($.s, r), $.b); const a = (t = p(t, $.t), t = v(t), ((r, e, t) => {
        const a = [K(r) / G, J]; const
            l = Y(a[0] * a[1]);
        for (let e = 0; e < a[0]; e++) for (let t = 0; t < G; t++) l[e * a[1] + t] = r[e * G + t];
        for (let r = 0; r < a[0]; r++) for (let t = 0; t < G; t++) l[r * a[1] + t + G] = e[r * G + t];
        return l;
    })(e = O(e, G, K(e) - G), t)); const l = $.j; const
        o = $.E;
    return t = p(((r, e) => {
        const t = [K(r) / J, J]; const a = [J, K(e) / J]; const l = K(e) / G; const o = [K(r) / J, K(e) / J]; const
            n = Y(o[0] * o[1]);
        for (let i = 0; i < o[0]; i++) {
            for (let u = 0; u < o[1]; u++) {
                let v = 0;
    ...
    const c = o(h(f(i, v), l)); const p = s(a, ((r) => {
        const e = [10, K(r) / 10]; const t = [e[0], e[1] - 1]; const
            a = Y(t[0] * t[1]);
        for (let o = 0; o < t[0]; o++) {
            const l = o * e[1];
    ...
    let t = m(r, $.i); const a = (t = u(t, $.W), t = s(u(t, e.z), X), C(t, e, 1)); const l = (t = s(u(t, a), X), w(t, e.w));
    const n = C(l, e, 2); const i = k(n, e); let f = w(n, i);
    const h = (f = g(f, $.l), f = u(f, $.Q), f = v(f), t = s(u(t, f), X), ((r) => {
        const e = $.c; const
            t = $.u;
        return o(u(m(r, e), t));
    })(i)); const d = C(t, e, 3); let
        c = (t = s(u(t, d), X), m(t, $.z));
    return [c = u(c, $.v), h];
}; const U = (r) => {
    const [e, t] = ((r, e) => {
        const t = []; let
            a = 0;
        for (let v = 0; v < K(e); v++) {
            const l = a; const o = a + e[v]; const
                n = O(r, l, o);
            a = o, t.push(n);
        }
        return t;
    })(r, [120, 1]); const a = o(t)[0]; const i = R() < a ? 1 : 0; var
        [f, c, p, w] = ((r, e) => {
            const t = []; let
                a = 0;
            for (let v = 0; v < K(e); v++) {
    ...
    p = v(p);
    const g = BIAS; var c = h(n(c), N(g)); var
        f = (f = l(d(f)), s(f, 1 + g));
    ...
    const b = ((r) => {
        ...
        return Y([t]);
    })(f); const M = x(w, b, 2); const y = x(c, b, 2); let C = x(p, b, 1); const A = y[0]; const k = y[1];
    let F = [A, (C = C[0]) * k, 0, k * P(1 - C * C)];
    F = Y(F);
    const U = ((r) => {
        const e = Y(r);
        for (let n = 0; n < r; n++) {
            const t = 1 - R(); const a = 1 - R(); const l = P(-2 * Q(t)); const
                o = Math.cos(2 * Math.PI * a);
            e[n] = l * o;
        }
        return e;
    })(2); const L = u(M, m(U, F)); let
        E = [L[0], L[1], i];
    return E = Y(E);
}; const L = (r, e) => {
    const [t, a] = F(r, e);
    ...
    for (let d = 0; d < K(r); d++) {
        if (d == 0) {
            var n = r[d + 1][0] - r[d][0];
            var v = r[d + 1][1] - r[d][1];
        } else if (d == K(r) - 1) n = r[d][0] - r[d - 1][0], v = r[d][1] - r[d - 1][1]; else n = r[d + 1][0] - r[d - 1][0], v = r[d + 1][1] - r[d - 1][1];
        let i = Math.sqrt(Math.pow(n, 2) + Math.pow(v, 2)); const u = (i = Math.max(i, 14), o[d] / e);
        var f = [(f = [t * (f = [-v / i, n / i])[0], t * f[1]])[0] / u, f[1] / u]; let s = r[d][0] + 2 * f[0];
        let h = r[d][1] + 2 * f[1];
        a.push([s, h]);
        s = r[d][0] - 2 * f[0], h = r[d][1] - 2 * f[1];
        l.push([s, h]);
    }
    const d = a.concat(l.reverse()); const c = [['M ', sr(d[0][0]), ',', sr(d[0][1])].join('')]; const
        p = K(d);
    for (let r = 0; r < p; r++) {
        const w = d[(r - 1 + p) % p]; const m = d[r]; const g = d[(r + 1) % p]; const b = d[(r + 2) % p]; const M = I(g, w); const y = I(b, m);
        const x = j(m, T(M, 0.2)); const C = I(g, T(y, 0.2));
    ...
    const o = [];
    for (let r = 0; r < K(e); r++) {
        const n = Math.max(r - 2, 0); const v = Math.min(r + 2 + 1, K(e)); let
            i = 0;
    ...
    if (K(a) != 0) {
        const l = STROKE_WIDTH; const
            o = z(a);
        for (let n = r; n < K(o); n++) {
            a = o[n];
            K(a) < 2 ? K(a) != 0 && a[0][2] != 1 || (r += 1) : (e && handwritingCanvas.removeChild(handwritingCanvas.lastChild), q(a, t, l), a[K(a) - 1][2] == 1 ? (e = !1, r += 1) : e = !0);
        }
    }
}; var j = (r, e) => [e[0] + r[0], e[1] + r[1]]; var I = (r, e) => [r[0] - e[0], r[1] - e[1]];
var T = (r, e) => [e * r[0], e * r[1]]; const V = (r, e, t) => {
    let a = 0; const l = []; const o = []; const
        n = [];
    for (let f = 0; f < K(r); f++) {
        const v = r[f];
        a += e[f];
        const i = Math.floor(a / t[1]); const
            u = a % t[1];
    ...
    const a = t.reduce((r, e) => r * e, 1); const l = Y(a); const o = K(r); let
        n = 0;
    for (let t = 0; t < o; t++) {
        const v = r[t];
        l[n += e[t]] = v;
    ...
    w: 24,
    y: 22,
}; var N = Math.exp; var P = Math.sqrt; var Q = Math.log; var R = Math.random; var W = (Math.abs, Math.max, Math.min, Math.floor);
var X = P(0.5); var G = 256; var J = 512; var K = (r) => r.length; var O = (r, e, t) => r.slice(e, t); var Y = function () {
    return new Float32Array(...arguments);
}; var Z = null; var $ = $; var rr = null; var er = null; var tr = []; var elementById = document.getElementById.bind(document);
const handwritingCanvas = elementById('handwritingCanvas');
